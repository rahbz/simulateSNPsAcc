...
hdf5=`readlink -f the1001genomes_filtered_all_chroms.hdf5`
hdf5_acc=`readlink -f the1001genomes_filtered_all_chroms.acc.hdf5`
kinFile=`readlink -f the1001genomes_filtered_all_chroms.kinship.ibs.hdf5`
totSNPs=`readlink -f the1001genomes_filtered_all_chroms.totalSNPcount.txt`
...
#kinFile=`readlink -f wholeImputed_filtered_all_chroms.kinship.ibs.hdf5`
#totSNPs=`readlink -f wholeImputed_filtered_all_chroms.totalSNPcount.txt`
...
nSNPs=(`echo "500,1000,2000,3000,4000,5000,8000,10000,20000,30000,50000,80000,100000,200000,300000,400000,500000,800000,900000,1000000,2000000,3000000,4000000,5000000,8000000,10000000" | sed 's/,/\n/g'`)
...
for (( i=0; i < $numAcc; i++ ))
do
	python ~/MyScripts/simulateSNPsAcc/07_SimulateSNPsAcc/01_SimulateSNPsperAcc.py -d $hdf5 -e $hdf5_acc -a $i -n $nSNP -o ${i}_$nSNP.ScoreAcc.txt -r ${i}_$nSNP.refScore.txt 2> logs/logs.$i.$numAcc > logs/logs.$i.$numAcc &
	if [ $(($i % 16)) = 0 ]
	then
		wait
	fi
...
# Make a CSV file
module load R
Rscript ~/MyScripts/simulateSNPsAcc/07_SimulateSNPsAcc/02_makeCSV_fromScoreAcc.R $hdf5 $PBS_O_WORKDIR/SNPs_$nSNP/intermediate_modified.csv
