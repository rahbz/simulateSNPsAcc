...
nSNPs=(`echo "100,200,300,400,500,800,1000,2000,3000,4000,5000,8000,10000,20000,30000,50000,80000,100000,200000,300000,500000" | sed 's/,/\n/g'`)
...
for (( i=0; i < $numAcc; i++ ))
do
	python ~/MyScripts/simulateSNPsAcc/04_SimulateSNPsAcc/01_SimulateSNPsperAcc.py -d $PBS_O_WORKDIR/$hdf5 -e $PBS_O_WORKDIR/$hdf5_acc -t $PBS_O_WORKDIR/$totSNPs -a $i -n $nSNP -o ${i}_$nSNP.ScoreAcc.txt -r ${i}_$nSNP.refScore.txt &
#	python ~/MyScripts/simulateSNPsAcc/04_SimulateSNPsAcc/01_SimulateSNPsperAcc.py -d $hdf5 -e $hdf5_acc -t $totSNPs -a $i -n $nSNP -o ${i}_$nSNP.ScoreAcc.txt -r ${i}_$nSNP.refScore.txt &
	if [ $(($i % 16)) = 0 ]
	then
		wait
	fi
...
# Make a CSV file
module load R
#Rscript ~/MyScripts/simulateSNPsAcc/04_SimulateSNPsAcc/02_makeCSV_fromScoreAcc.R $PBS_O_WORKDIR/SNPs_$nSNP $PBS_O_WORKDIR/SNPs_$nSNP/intermediate_modified.csv $hdf5
Rscript ~/MyScripts/simulateSNPsAcc/04_SimulateSNPsAcc/02_makeCSV_fromScoreAcc.R $PBS_O_WORKDIR/SNPs_$nSNP $PBS_O_WORKDIR/SNPs_$nSNP/intermediate_modified.csv $PBS_O_WORKDIR/$hdf5
