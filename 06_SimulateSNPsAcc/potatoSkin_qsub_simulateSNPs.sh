...
nSNPs=(`echo "100,200,300,400,500,800,1000,2000,3000,4000,5000,8000,10000,20000,30000,50000,80000,100000,200000,300000,400000,500000,800000,900000,1000000,2000000,3000000,4000000,5000000,8000000,10000000" | sed 's/,/\n/g'`)
#nSNPs=(`echo "800000,900000,1000000,2000000,3000000,4000000,5000000,8000000,10000000" | sed 's/,/\n/g'`)
...
for (( i=0; i < $numAcc; i++ ))
do
	python ~/MyScripts/simulateSNPsAcc/06_SimulateSNPsAcc/01_SimulateSNPsperAcc.py -d $PBS_O_WORKDIR/$hdf5 -e $PBS_O_WORKDIR/$hdf5_acc -a $i -n $nSNP -o ${i}_$nSNP.ScoreAcc.txt -r ${i}_$nSNP.refScore.txt > $PBS_O_WORKDIR/logs/logs.$i.$numAcc 2> $PBS_O_WORKDIR/logs/logs.$i.$numAcc &
#	python ~/MyScripts/simulateSNPsAcc/06_SimulateSNPsAcc/01_SimulateSNPsperAcc.py -d $hdf5 -e $hdf5_acc -a $i -n $nSNP -o ${i}_$nSNP.ScoreAcc.txt -r ${i}_$nSNP.refScore.txt $PBS_O_WORKDIR/logs/logs.$i.$numAcc > $PBS_O_WORKDIR/logs/logs.$i.$numAcc &
	if [ $(($i % 16)) = 0 ]
	then
		wait
	fi
...
Rscript ~/MyScripts/simulateSNPsAcc/06_SimulateSNPsAcc/02_makeCSV_fromScoreAcc.R $PBS_O_WORKDIR/SNPs_$nSNP $PBS_O_WORKDIR/SNPs_$nSNP/intermediate_modified.csv $PBS_O_WORKDIR/$hdf5
